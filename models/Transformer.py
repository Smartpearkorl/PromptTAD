...
def torch_attention(query, key, value, masksize, dropout=None):
    mask = None
    if masksize != 1:
        masksize = int(masksize / 2)
        query_len = query.shape[-2]
        key_len = key.shape[-2]
        mask = torch.zeros(query_len, key_len, device=query.device)  # 初始为全0
        for i in range(query_len):
            start = max(0, i - masksize)
            end = min(key_len, i + masksize + 1)
            mask[i, start:end] = 1  # 允许关注窗口内的位置
        mask = mask.bool()  # 转换为布尔掩码（True表示保留）
    out = torch.nn.functional.scaled_dot_product_attention(
        query, key, value, attn_mask=mask, dropout_p=dropout.p if dropout else 0.0
    )
    return out, None
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value):
        ...
        # x, self.attn = torch_attention(query, key, value, self.masksize, dropout=self.dropout)
